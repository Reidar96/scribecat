ScribeCat - portable build
==========================

This is the same application as the installer version, but it does not install
anything: run ScribeCat.exe and you are done. No administrator rights, no
...
means portable mode cannot start and ScribeCat falls back to your user profile
...
Everything ScribeCat owns is kept in the ".scribecat" folder next to
ScribeCat.exe: your settings, the AI configuration, keyboard shortcuts, the
...
The file ".scribecat/portable" is what switches portable mode on. Delete it and
ScribeCat behaves like the installed version, storing its settings in your
...
- If you put ScribeCat somewhere you cannot write to (C:\Program Files, a
  read-only medium), portable mode cannot start. ScribeCat then falls back to
  your user profile and says so in Settings.
- The portable build does not update itself. It still tells you when a new
  version is out; download the new ZIP and replace ScribeCat.exe.
...
Antivirus software may go a step further and scan ScribeCat.exe on every launch,
...
https://github.com/snooky234/scribecat
