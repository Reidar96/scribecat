...
DATA_DIR="${SCRIBECAT_VAULT_PATH:-/data}"
...
case "$PUID$PGID" in
  *[!0-9]*) echo "[scribecat] PUID and PGID must be numeric (got PUID=$PUID PGID=$PGID)." >&2; exit 1 ;;
esac
...
if ! getent group "$PGID" >/dev/null 2>&1; then
  addgroup -g "$PGID" scribecat
fi
GROUP_NAME="$(getent group "$PGID" | cut -d: -f1)"

if ! getent passwd "$PUID" >/dev/null 2>&1; then
  adduser -D -H -u "$PUID" -G "$GROUP_NAME" scribecat
fi
...
if [ -n "$(find "$DATA_DIR" ! -user "$PUID" -print -quit 2>/dev/null)" ]; then
  echo "[scribecat] handing $DATA_DIR to uid $PUID / gid $PGID"
